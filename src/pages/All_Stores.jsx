...
const All_Stores = ({
    ...
    const [statusFilter, setStatusFilter] = useState(() => {
        if (location.state?.initialStatusFilter && Array.isArray(location.state.initialStatusFilter)) {
    ...
    const [planFilter, setPlanFilter] = useState(() => {
        if (location.state?.initialPlanFilter && Array.isArray(location.state.initialPlanFilter)) {
    ...
    const filteredAndSortedDiscounts = useMemo(() => {
        ...
        if (search) {
            ...
            result = result.filter(item => 
                ...
                (item.storeEmail || '').toLowerCase().includes(query) ||
        ...
        if (statusFilter && statusFilter.length > 0) {
            result = result.filter(item => {
                const rawEvents = Array.isArray(item?.pastEvents) ? item.pastEvents : [];
                let latestEventName = '';
                ...
                const isClosed = lowerEvent.includes('close') || lowerEvent.includes('closed') || item?.isStoreClosed;
                const isUninstall = lowerEvent.includes('uninstall') || lowerEvent.includes('uninstalled') || item?.isActive === false;
        ...
        // 3. Plan filter
        if (planFilter && planFilter.length > 0) {
            result = result.filter(item => planFilter.includes(item.plan || 'No Plan'));
        }
                ...
                if (json.success && json.data) {
                    ...
                    if (json.data.stores && Array.isArray(json.data.stores)) {
                        ...
                        for (const ev of json.data.events) {
                            const shopId = ev.shop?.id || ev.shop?.myshopifyDomain || 'unknown';
                            const domain = ev.shop?.myshopifyDomain || (shopId !== 'unknown' ? shopId.replace('gid://partners/Shop/', '') + '.myshopify.com' : 'unknown.myshopify.com');
                            if (!storeMap[domain]) {
                            ...
                            let eventLabel = ev.type.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (l) => l.toUpperCase());
                            if (ev.type === 'RELATIONSHIP_INSTALLED') eventLabel = 'Installed';
                            else if (ev.type === 'RELATIONSHIP_UNINSTALLED') eventLabel = 'Uninstalled';
                            else if (ev.type === 'RELATIONSHIP_REACTIVATED') eventLabel = 'Reopened';
                            else if (ev.type === 'RELATIONSHIP_DEACTIVATED') eventLabel = 'Store Closed';

                            store.pastEvents.push({ eventName: eventLabel, type: ev.type, timestamp: ev.occurredAt });
                            if (new Date(ev.occurredAt) < new Date(store.createdAt)) store.createdAt = ev.occurredAt;
                        ...
                        storesList = Object.values(storeMap).sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));
                    } else {
